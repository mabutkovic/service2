            ...
            parallel {
                stage('Project1') {
                    stages {
                        ...
                        stage('Deploy to Production') {
                            steps {
                                sh(script: "source /etc/profile; helm upgrade --set version=$VERSION_NUMBER --set SERVICE1_URL=http://project1-staging-service1-svc:8080 -f service2-workflow/values.p1.prod.yaml p1-prod-service2-workflow ./service2-workflow")
                            }
                        }
                    }
                }
                stage('Project2') {
                    stages {
                        ...
                        stage('Deploy to Production') {
                            steps {
                                sh(script: "source /etc/profile; helm upgrade --set version=$VERSION_NUMBER --set SERVICE1_URL=http://project2-staging-service1-svc:8080 -f service2-workflow/values.p2.prod.yaml p2-prod-service2-workflow ./service2-workflow")
                            }
                        }
                    }
                }
